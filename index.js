...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("TASHINDU-MD~")) {
    console.log("❌ Invalid TASHINDU-MD session format");
    process.exit(1);
  }
  
  const decoded = Buffer.from(session.substring(12), 'base64').toString('utf8');
  JSON.parse(decoded);
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'TASHINDU'}</p>
        </div>
      </body>
      </html>
    `);
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://ibb.co/20Tc9jDz'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 TASHINDU-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ TASHINDU-MD Bot connected to WhatsApp!");
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://ibb.co/20Tc9jDz');
